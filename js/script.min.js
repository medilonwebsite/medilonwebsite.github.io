...
jQuery(function($){new WOW().init();});jQuery(document).ready(function(){$('body').scrollspy({target:'.navbar-collapse',offset:195});$(window).on('scroll',function(){if($(window).scrollTop()>200){$('.fixed-top').addClass('menu-bg');}else{$('.fixed-top').removeClass('menu-bg');}});});var headerEl=document.querySelector("header");window.addEventListener("scroll",function(){var height=headerEl.getBoundingClientRect().height;if(window.pageYOffset-height>570){if(!headerEl.classList.contains("top-bar")){headerEl.classList.add("top-bar");}}else{headerEl.classList.remove("top-bar");}});const glide=new Glide(".glide");const captionsEl=document.querySelectorAll(".slide-caption");glide.on(["mount.after","run.after"],function(){const caption=captionsEl[glide.index];anime({targets:caption.children,opacity:[0,1],duration:400,easing:"linear",delay:anime.stagger(400,{start:300}),translateY:[anime.stagger([40,10]),0]});});glide.on("run.before",function(){document.querySelectorAll(".slide-caption > *").forEach(function(el){el.style.opacity=0;});});glide.mount();const staggeringOption={delay:300,distance:"50px",duration:500,easing:"ease-in-out",origin:"bottom"};ScrollReveal().reveal(".service-block ",{...staggeringOption,interval:350});ScrollReveal().reveal(".service-item",{...staggeringOption,interval:350});ScrollReveal().reveal(".contact-meta-block",{...staggeringOption,interval:350});